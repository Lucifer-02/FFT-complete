...
complex c_divide(complex a, float b)  // divide a complex number with a float number
{
    complex c;
    c.real = a.real / b;
    c.img = a.img / b;
...
void FFT(float *x, complex *X, int N)   // Fast Fourier Transform, return FFT of x signal
{
    if (N==1) return;
...
int main()
{
    ...
    for(k = 0; k<N; k++)
    {
        P[k] = sqrt(X_oneside[k].img*X_oneside[k].img + X_oneside[k].real*X_oneside[k].real);
    }
    ...
    fprintf(f, "\n");
    for (k = 0; k < N_oneside ; k++) fprintf(f,"%f,", P[k]);
